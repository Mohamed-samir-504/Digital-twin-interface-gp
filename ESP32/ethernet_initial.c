...
static SemaphoreHandle_t read_alerts_sem;
...
void parse_double(const char *str, MQTT_MESSAGE *mqtt) 
{
    char *ptr = NULL;
    mqtt->throttle.f32 = (float)strtod(str, &ptr);   
    mqtt->steering.f32 = (float)strtod(ptr, NULL);
}
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    switch ((esp_mqtt_event_id_t)event_id) 
    {
        ...
        case MQTT_EVENT_CONNECTED:
            ...
            msg_id = esp_mqtt_client_subscribe(client, "readings/steering", 0);
            ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);

            msg_id = esp_mqtt_client_subscribe(client, "readings/throttle",0);
            ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);
        ...
        case MQTT_EVENT_DATA:
            ESP_LOGI(TAG, "MQTT_EVENT_DATA : Recieved a publish message !");
            ...
            if(!strncmp(event->topic,"readings/steering",event->topic_len)){
                tx_frame.canid = ID_CTRL_STEERING;
            }else{
                tx_frame.canid = ID_CTRL_THROTTLE;
            }

            /* Convert string to float */
    
/*          tx_frame.steering.bytes.u8_1 = event->data[0];
            tx_frame.steering.bytes.u8_2 = event->data[1];
            tx_frame.steering.bytes.u8_3 = event->data[2];
            tx_frame.steering.bytes.u8_4 = event->data[3];

            tx_frame.throttle.bytes.u8_1 = event->data[4];
            tx_frame.throttle.bytes.u8_2 = event->data[5];
            tx_frame.throttle.bytes.u8_3 = event->data[6];
            tx_frame.throttle.bytes.u8_4 = event->data[7];
 */
            
            if(xQueueSend(frame_queue_tx,(void *) &tx_frame,0) != pdPASS){
                ESP_LOGE(TAG,"Error, Queue should never be full!");
            }
...
static void mqtt_app_start(void)
{
    esp_mqtt_client_config_t mqtt_cfg = {
        .broker.address.uri = "mqtt://192.168.1.3:1883",
    };
...
void twai_receive_task(void *param)
{
    ...
    while(1)
    {
        ...
        if (status == ESP_OK) {
            printf("twai_receive_task : Msg received - Data = ");
            ...
            printf("Steering : %f \t Throttle: %f \n", data.steering.f32, data.throttle.f32);

            /* Publish recieved message to mqtt */
            // if(rx_message.identifier == ID_CTRL_STEERING){
...
void twai_transmit_task(void * param)
{
    ...
    ESP_LOGI(TAG, "TWAI state 0x%X\n", status.state);

    twai_message_t tx_msg = {
        .data_length_code = 8, 
        .identifier = ID_CTRL_STEERING,
    ...
    for (;;)
    {
        ...
        {
            tx_msg.data[0]= tx_frame.steering.bytes.u8_1 ;
            tx_msg.data[1]= tx_frame.steering.bytes.u8_2 ;
            tx_msg.data[2]= tx_frame.steering.bytes.u8_3 ;
            tx_msg.data[3]= tx_frame.steering.bytes.u8_4 ;

            tx_msg.data[4]= tx_frame.throttle.bytes.u8_1 ;
            tx_msg.data[5]= tx_frame.throttle.bytes.u8_2 ;
            tx_msg.data[6]= tx_frame.throttle.bytes.u8_3 ;
            tx_msg.data[7]= tx_frame.throttle.bytes.u8_4 ;
            ...
            if(twai_status == ESP_OK){
                ...
                printf("Failed to queue message for transmission %x\n",twai_status);
            }

        }else{
            ESP_LOGE(TAG,"Error Queue should never wakeup from block.");
        }
...
void app_main(void)
{
    ...
    read_alerts_sem = xSemaphoreCreateBinary();
    ...
    xTaskCreatePinnedToCore(twai_alerts_task,   "TWAI_alerts", 4096, NULL, ALERT_TSK_PRIO, NULL, tskNO_AFFINITY);
    ...
    xSemaphoreGive(read_alerts_sem);
